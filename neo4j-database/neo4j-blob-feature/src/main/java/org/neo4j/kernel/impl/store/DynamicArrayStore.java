...
public class DynamicArrayStore extends AbstractDynamicStore
{
    ...
    //NOTE: blob
    private static void allocateFromBlob( Collection<DynamicRecord> target, BlobEntry[] array,
            ...
            DynamicRecordAllocator recordAllocator, boolean allowStorePointsAndTemporal )
    {
        ...
        //NOTE: blob
        if ( type.equals( BlobEntry.class ) )
        {
            allocateFromBlob( target, (BlobEntry[]) array, recordAllocator );
        }
    ...
    public static Value getRightArray( Pair<byte[],byte[]> data )
    {
        ...
        else if ( typeId == PropertyType.BLOB.intValue() )
        {
            ...
            int arrayLength = headerBuffer.getInt();
            ByteBuffer dataBuffer = ByteBuffer.wrap( bArray );
            BlobEntry[] result = StoreBlobIO.readBlobArray( dataBuffer, arrayLength );
